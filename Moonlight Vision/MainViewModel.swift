...
class MainViewModel: NSObject, ObservableObject, DiscoveryCallback, PairCallback, AppAssetCallback {
    ...
    private let connectionCallbacks: ConnectionCallbacksImpl
    private let controllerSupportDelegate: ControllerSupportDelegateImpl
    
    private let sampleBufferVideoRenderer: AVSampleBufferVideoRenderer
    
    let curvedScreenBuilder: CurvedScreenBuilder
    
    var currentStreamConfig = StreamConfiguration()
    var currentStreamManager: StreamManager?
    ...
    override init() {
        ...
        self.curvedScreenBuilder = CurvedScreenBuilder()
        self.controllerSupportDelegate = ControllerSupportDelegateImpl()
        self.connectionCallbacks = ConnectionCallbacksImpl()
        self.sampleBufferVideoRenderer = AVSampleBufferVideoRenderer()
        self.videoMaterial = VideoMaterial(videoRenderer: sampleBufferVideoRenderer)
    ...
    nonisolated func updateStatus(_ status: String?) {
        Task { @MainActor in
            self.status = status
        }
    }
    ...
    func stream(app: TemporaryApp) {
        ...
        controllerSupport = ControllerSupport(
            config: config,
            delegate: controllerSupportDelegate
        )
        
        sampleBufferVideoRenderer.flush(removingDisplayedImage: true)
        
        guard let streamManager = StreamManager(
            config: config,
            renderView: nil,
            sampleBufferVideoRenderer: sampleBufferVideoRenderer,
            connectionCallbacks: connectionCallbacks
        )
        else { return }
        
        setCurrent(config: config, manager: streamManager, app: app)
        updateStatus("Starting \(config.appName ?? "")...")
        
        let queue = OperationQueue()
        queue.addOperation(streamManager)
    ...
    func setCurrent(
        ...
        currentStreamManager = manager
        currentStreamingApp = app
    }
    
    func stopStream() {
        currentStreamManager?.stopStream()
        controllerSupport?.cleanup()
        currentStreamingApp = nil
    }
