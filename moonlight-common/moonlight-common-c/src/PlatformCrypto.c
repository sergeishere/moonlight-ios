...
// CommonCrypto GCM oneshot functions (exported but not in public header)
        ...
        *outputDataLength = inputDataLength;
        ...
        if (!ctx->initialized) {
            CCCryptorStatus status = CCCryptorCreateWithMode(
                kCCEncrypt, kCCModeCBC, kCCAlgorithmAES128, ccNoPadding,
                iv, key, (size_t)keyLength,
                NULL, 0, 0, 0,
                &ctx->cryptor);
        ...
        CCCryptorStatus status = CCCryptorUpdate(
            ctx->cryptor,
            inputData, (size_t)inputDataLength,
            outputData, (size_t)*outputDataLength,
            &dataOutMoved);
        if (status != kCCSuccess) {
            return false;
        }

        *outputDataLength = (int)dataOutMoved;
    ...
    *outputDataLength = outLength;
    ...
    if (algorithm == ALGORITHM_AES_GCM) {
        ...
        *outputDataLength = inputDataLength;
        ...
        if (!ctx->initialized) {
            CCCryptorStatus status = CCCryptorCreateWithMode(
                kCCDecrypt, kCCModeCBC, kCCAlgorithmAES128, ccNoPadding,
                iv, key, (size_t)keyLength,
                NULL, 0, 0, 0,
                &ctx->cryptor);
            if (status != kCCSuccess) {
...
PPLT_CRYPTO_CONTEXT PltCreateCryptoContext(void) {
    PPLT_CRYPTO_CONTEXT ctx = (PPLT_CRYPTO_CONTEXT)calloc(1, sizeof(*ctx));
    if (!ctx) {
        return NULL;
    }
...
#elif defined(__APPLE__)
    // Nothing to pre-allocate; cryptor created lazily for CBC
#else
